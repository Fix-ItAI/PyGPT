...
# Create the reverse mapping (string to index)
...
context = torch.tensor([stoi[ch] for ch in prompt] if prompt else [], dtype=torch.long, device=device).unsqueeze(0) # shape (1, T)
